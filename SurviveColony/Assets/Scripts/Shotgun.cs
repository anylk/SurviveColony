...
public class Shotgun : Weapon
{
    ...
    public override void Fire()
    {
        ...
        float angleBetweenBrust = brustAngle / (brustCount - 1);

        for (int i = 0; i < brustCount; i++)
        {
            Quaternion rotation = Quaternion.AngleAxis(-brustAngle / 2f + i * angleBetweenBrust, Vector3.up);
            Vector3 beamDirection = rotation * raycastOrigin.forward;
            Vector3 velocity = beamDirection * bulletSpeed;
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        for (int i = 0; i < brustCount; i++)
        {
            Quaternion rotation = Quaternion.AngleAxis(-brustAngle / 2f + i * angleBetweenBrust, Vector3.up);
            Vector3 beamDirection = rotation * raycastOrigin.forward;
            Gizmos.DrawRay(raycastOrigin.position, beamDirection * float.MaxValue);
        }
    }
